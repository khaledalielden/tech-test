...
static TradeListX* fxTradeList = nullptr;

void setUpFx() {
     //   std::cout << "size " << fxTradeList << std::endl;
    ...
    auto trades = loader.loadTrades();
       // std::cout << "Fxsize " << trades.size() << std::endl;
    fxTradeList = new TradeListX();
...
TEST(TestFxTradeLoadCount) {
    setUpFx();
  //  std::cout << "size " << fxTradeList->size() << std::endl;
    ASSERT_EQ(fxTradeList->size(), 5);
}
...
TEST(TestFxTradeLoadAccuracyOfFirstTrade) {
    ...
    ASSERT_NEAR(trade->getAmount(), 145000000.0, 0.01);
...
TEST(TestFxTradeLoadAccuracyOfLastTrade) {
    ...
    ASSERT_NEAR(trade->getAmount(), 1223445000.0, 0.01);
